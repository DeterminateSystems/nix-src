...
void Store::substitutePaths(const StorePathSet & paths)
{
    ...
    queryMissing(paths2, willBuild, willSubstitute, unknown, downloadSize, narSize);

    if (!willSubstitute.empty())
        try {
            std::vector<DerivedPath> subs;
            for (auto & p : willSubstitute)
...
static Derivation readDerivationCommon(Store & store, const StorePath & drvPath, bool requireValidPath)
{
    ...
    try {
        return parseDerivation(
            store, accessor->readFile(CanonPath(store.printStorePath(drvPath))), Derivation::nameFromPath(drvPath));
    } catch (FormatError & e) {
