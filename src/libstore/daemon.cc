    ...
    WorkerProto::Op op)
{
    ...
    case WorkerProto::Op::AddToStore: {
        if (conn.protoVersion >= WorkerProto::Version{.number = {1, 25}}) {
            ...
            auto provenance = conn.protoVersion.features.contains(WorkerProto::Version::featureProvenance)
    ...
    case WorkerProto::Op::AddToStoreNar: {
        ...
        info.provenance = conn.protoVersion.features.contains(WorkerProto::Version::featureProvenance)
                              ? Provenance::from_json_str_optional(readString(conn.from))
                              : nullptr;
        conn.from >> repair >> dontCheckSigs;
        if (!trusted && dontCheckSigs)
...
void processConnection(ref<Store> store, FdSource && from, FdSink && to, TrustedFlag trusted, RecursiveFlag recursive)
{
    ...
    if (!experimentalFeatureSettings.isEnabled(Xp::Provenance))
        version.features.erase(std::string(WorkerProto::Version::featureProvenance));
