    ...
    std::shared_ptr<const Provenance> provenance)
{
    ...
    if (conn->protoVersion >= WorkerProto::Version{.number = {1, 25}}) {
        ...
        conn->to << repair;
        if (conn->protoVersion.features.contains(WorkerProto::Version::featureProvenance))
...
{
    auto conn(getConnection());
    if (!conn->protoVersion.features.count(WorkerProto::Version::featureQueryActiveBuilds))
